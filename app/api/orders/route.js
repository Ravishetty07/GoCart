...
export async function POST(request) {
  try {
    ...
    // ✅ Razorpay Payment Flow
      ...
      // Orders already created in DB; now client will create Razorpay order via /api/razorpay/create-order
      // and verify later using /api/razorpay/verify-payment
      return NextResponse.json({
        message: "Razorpay order(s) created successfully",
        orderIds,
        fullAmount,
      });
    }
    ...
    const orders = await prisma.order.findMany({
      where: {
        ...
        OR: [
          ...
          { AND: [{ paymentMethod: PaymentMethod.RAZORPAY }, { paymentStatus: "PAID" }] },
        ],
      },
      include: {
